...
function hoot_lib_constants() {
	...
	if ( is_child_theme() ) {
		...
		if ( is_object( hoot_data( 'theme' )->parent() ) ) {
			...
			hoot_set_data( 'template_name',       'Strute' );
	...
	hoot_set_data( 'template_slug', $template_slug );
	if ( is_child_theme() ) {
		$childtheme_slug = strtolower( preg_replace( '/[^a-zA-Z0-9]+/', '_', trim( hoot_data()->childtheme_name ) ) );
		hoot_set_data( 'childtheme_slug', $childtheme_slug );
	}
...
function hoot_lib_core() {
	...
	/* Load the Scroller */
	require_once( hoot_data()->libdir . 'scroller.php' );
