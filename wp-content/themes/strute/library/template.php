...
function hoot_comments_callback( $comment ) {
	...
	if ( !empty( $template ) && is_array( $template ) )
		hoot_locate_template( $template, true, false );
...
function hoot_get_wp_link() {
	return sprintf( '<a class="wp-link" href="%s">%s</a>', esc_url( __( 'https://wordpress.org', 'strute' ) ), esc_html__( 'WordPress', 'strute' ) );
}
endif;
...
function hoot_get_wp_theme_link( $link = '', $anchor = '' ) {
	...
	$title  = sprintf( __( '%s WordPress Theme', 'strute' ), hoot_data()->template_name );
...
function hoot_meta_info( $args = '', $context = '', $bool = false ) {
	...
	if ( is_array( $args ) ) {
		foreach ( array( 'author', 'date', 'cats', 'tags', 'comments' ) as $key ) {
			if ( in_array( $key, $args ) ) $display[ $key ] = true;
		}
	} elseif ( is_string( $args ) ) {
		$args = hoot_sortlist( $args );
		if ( is_array( $args ) ) {
			foreach ( $args as $key => $keyarr ) {
				if (
					in_array( $key, array( 'author', 'date', 'cats', 'tags', 'comments' ) ) &&
					( empty( $keyarr ) || !is_array( $keyarr ) || empty( $keyarr['sortitem_hide'] ) )
				)
				$display[ $key ] = true;
			}
		}
...
function hoot_display_meta_info( $args = '', $context = '', $editlink = true ) {
	...
	foreach ( $display as $key => $show ) { if ( $show ) {
		if ( $key === 'author' ) {
			$blocks['author']['label'] = __( 'By:', 'strute' );
			ob_start();
			the_author_posts_link();
			$blocks['author']['content'] = '<span ' . hoot_get_attr( 'entry-author' ) . '>' . ob_get_clean() . '</span>';
		} elseif ( $key === 'date' ) {
			$blocks['date']['label'] = __( 'On:', 'strute' );
			$blocks['date']['content'] = '<time ' . hoot_get_attr( 'entry-published' ) . '>' . get_the_date() . '</time>';
		} elseif ( $key === 'cats' ) {
			$category_list = get_the_category_list(', ');
			if ( !empty( $category_list ) ) :
				$blocks['cats']['label'] = __( 'In:', 'strute' );
				$blocks['cats']['content'] = $category_list;
			endif;
		} elseif ( $key === 'tags' ) {
			$blocks['tags']['label'] = __( 'Tagged:', 'strute' );
			$blocks['tags']['content'] = ( ! get_the_tags() ) ? __( 'No Tags', 'strute' ) : get_the_tag_list( '', ', ', '' );
		} elseif ( $key === 'comments' && comments_open() ) {
			ob_start();
			comments_popup_link(__( '0 Comments', 'strute' ),
								__( '1 Comment', 'strute' ),
								__( '% Comments', 'strute' ), 'comments-link', '' );
			$comments = ob_get_clean();
			if ( !empty( $comments ) ) {
				$blocks['comments']['label'] = __( 'With:', 'strute' );
	...
	if ( $editlink && $edit_link = get_edit_post_link() ) :
		...
		$blocks['editlink']['content'] = '<a href="' . $edit_link . '">' . __( 'Edit This', 'strute' ) . '</a>';
	endif;
...
function hoot_developer_data( $commented = true ) {
	ob_start();
	echo intval( get_num_queries() ) . ' ' . esc_attr__( 'queries.', 'strute' ) . ' ';
	timer_stop(1);
	echo esc_html( __( ' seconds. ', 'strute' ) . ( memory_get_peak_usage(1) / 1024 ) / 1024 . 'MB' );
	$out = ob_get_contents();
	ob_end_clean();
	return $out;
}
