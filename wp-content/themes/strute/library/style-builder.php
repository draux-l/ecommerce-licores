...
/**
 * Wrapper function to add general CSS vars with Style-Builder class
 * $hoot_style_builder->add will sanitizes the property and adds it using 'wp_add_inline_style()'
 * @since 3.0.0
 * @access public
 * @param array $cssvars css vars array
 * @return void
 */
function hoot_add_cssvars( $cssvars ) {
	global $hoot_style_builder;
	$hoot_style_builder->add_cssvars( $cssvars );
}
...
class Hoot_Style_Builder {
	...
	/**
	 * The array for storing css variables.
	 *
	 * @since 3.0.0
	 * @access private
	 * @var array Holds the css vars array.
	 */
	...
	function build_css(){
		 ...
		 * @2 : Parent lite
		 *      @3 : Child lite (+dark if any)
		 * @4 : Parent premium prepare (rules removed)
		 * @5 : Parent premium
		 *      @6 : Child premium prepare (rules removed)
		 *      @6 : Child premium
		 * @6 : Parent HK lite
		 * @7 : Parent HK premium
		 * @8 : Parent blocks
		...
		if ( !empty( $this->cssvars ) ) {
			$css .= $this->build_css_string( array( ':root' => $this->cssvars ), $n, $t );
		}
	...
	/**
	 * Sanitize and add CSS vars to cssvars array.
	 * @since 3.0.0
	 * @access public
	 * @param array $cssvars
	 * @return void
	 */
	function add_cssvars( $cssvars ){
		// Sanitize and add vars
		if ( is_array( $cssvars ) ) {
			foreach ( $cssvars as $key => $value ) {
				$skey = sanitize_key( $key );
				if ( $skey && !empty( $value ) ) {
					if ( is_scalar( $value ) ) {
						$this->cssvars[ $skey ] = wp_strip_all_tags( $value );
					} elseif ( is_array( $value ) ) {
						foreach ( $value as $device => $deviceval ) {
							switch ( $device ) {
								case 'desktop':
									$this->cssvars[ $skey ] = wp_strip_all_tags( $deviceval );
									break;
								case 'tablet':
									$default_breakpoint = 969;
									$breakpoint = apply_filters( 'hoot_dynamiccss_breakpoint', $default_breakpoint, 'tablet', $skey, $value );
									$breakpoint = intval( $breakpoint );
									$breakpoint = ( $breakpoint > 0 ) ? $breakpoint : $default_breakpoint;
									$mquery = 'only screen and (max-width: ' . $breakpoint . 'px)';
									if ( empty( $this->mediarules[ $mquery ] ) )
										$this->mediarules[ $mquery ] = array( ':root' => array() );
									elseif ( empty( $this->mediarules[ $mquery ][':root'] ) )
										$this->mediarules[ $mquery ][':root'] = array();
									$this->mediarules[ $mquery ][':root'][ $skey ] = wp_strip_all_tags( $deviceval );
									break;
								case 'mobile':
									$default_breakpoint = empty( $value['tablet'] ) ? 969 : 600;
									$breakpoint = apply_filters( 'hoot_dynamiccss_breakpoint', $default_breakpoint, 'mobile', $skey, $value );
									$breakpoint = intval( $breakpoint );
									$breakpoint = ( $breakpoint > 0 ) ? $breakpoint : $default_breakpoint;
									$mquery = 'only screen and (max-width: ' . $breakpoint . 'px)';
									if ( empty( $this->mediarules[ $mquery ] ) )
	...
	function sanitize_backgroundarray( $backgroundarray, $idtag = '', $important = false ) {
		...
		if ( 'custom' == $type ) {
			...
			if ( $pattern ) {
				$background_image = esc_url( $pattern );
