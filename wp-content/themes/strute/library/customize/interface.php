...
function hoot_customize_register( $wp_customize ) {
	...
	// Debug mode
	if ( isset( $_GET['customizer-debug'] ) ) {
		$setdebug =  empty( $_GET['customizer-debug'] ) ? 0 : (int) $_GET['customizer-debug'];
		set_theme_mod( 'customizer_debug', $setdebug );
	}
	$setdebug = (int) get_theme_mod( 'customizer_debug', 0 );
	...
	foreach ( $options['settings'] as $id => $setting ) :
		if ( isset( $setting['type'] ) ) :
			...
			if ( $setdebug ) {
				if ( isset( $setting['transport'] ) ) unset( $setting['transport'] );
...
function hoot_customize_preview_js() {
	...
	$script_uri = hoot_locate_script( hoot_data()->incuri . 'admin/js/customize-preview' );
	if ( $script_uri )
