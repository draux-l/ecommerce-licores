	...
	public function render_content() {
		...
		switch ( $this->type ) {
			...
			case 'note' :
				...
				echo '<div class="' . ( !empty( $this->class ) ? sanitize_html_class( $this->class ) : '' ) . ( $this->type == 'note' || $this->type == 'pnote' ? ' hootpnote' : '' ) . '">';
...
function hoot_customize_content_control_interface ( $wp_customize, $id, $setting ) {
	if ( isset( $setting['type'] ) ) :
		if ( $setting['type'] == 'content' || $setting['type'] == 'pnote' || $setting['type'] == 'pnote' ) {
