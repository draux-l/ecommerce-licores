 ...
 * @return void
 */
if ( !function_exists( 'strute_woo_setup' ) ) :
function strute_woo_setup() {
...
add_action( 'wp', 'strute_woo_setup' );
 ...
 * @return void
 */
if ( !function_exists( 'strute_woo_register_sidebars' ) ) :
function strute_woo_register_sidebars() {
	hoot_register_sidebar(
		array(
			'id'          => 'hoot-woo-sidebar-primary',
			'name'        => _x( 'Woocommerce Primary Sidebar', 'sidebar', 'strute' ),
			'description' => __( 'The primary sidebar for woocommerce pages.', 'strute' )
		)
	);
	hoot_register_sidebar(
		array(
			'id'          => 'hoot-woo-sidebar-secondary',
			'name'        => _x( 'Woocommerce Secondary Sidebar', 'sidebar', 'strute' ),
			'description' => __( 'The secondary sidebar for woocommerce pages (if you are using a 3 column layout with 2 sidebars).', 'strute' )
		)
	);
}
endif;
add_action( 'widgets_init', 'strute_woo_register_sidebars' );
 ...
 * @return array
 */
if ( !function_exists( 'strute_woo_attr_sidebar' ) ) :
function strute_woo_attr_sidebar( $attr, $context ) {
...
add_filter( 'hoot_attr_sidebar', 'strute_woo_attr_sidebar', 11, 2 );
 ...
 * @return array
 */
if ( !function_exists( 'strute_woo_layout' ) ) :
function strute_woo_layout( $sidebar ) {
	...
	$forcenosidebar = apply_filters( 'strute_woo_pages_force_nosidebar', true );
...
add_filter( 'strute_layout', 'strute_woo_layout' );
 ...
 * @param string $context
 */
if ( !function_exists('strute_woo_meta_info_blocks_display') ) :
function strute_woo_meta_info_blocks_display( $display, $context ) {
	if ( is_woocommerce() || is_cart() || is_checkout() || is_account_page() )
		$display = array();
	return $display;
}
endif;
add_filter( 'hoot_meta_info', 'strute_woo_meta_info_blocks_display', 10, 2 );
 ...
 * @return void
 */
if ( !function_exists( 'strute_woo_loop_hide_product_meta' ) ) :
function strute_woo_loop_hide_product_meta() {
