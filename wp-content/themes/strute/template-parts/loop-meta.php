 ...
 */
$display_loop_meta = apply_filters( 'strute_display_loop_meta', true );
do_action( 'strute_loop_meta', 'start' );
...
if ( !is_front_page() && !is_singular() ) :
	...
	$display_title = apply_filters( 'strute_loop_meta_display_title', true, 'plural' );
	if ( $display_title !== 'hide' ) :
		...
		<div <?php hoot_attr( 'loop-meta-wrap', 'archive', $wrapclasses ); ?>>
				...
				<div <?php hoot_attr( 'loop-meta', 'archive', 'hgrid-span-12' ); ?>>
					...
					<?php if ( is_author() ) : ?>
						<div class="loop-meta-gravatar"><?php
							...
							$gwidth = apply_filters( 'strute_loop_meta_gravatar', 0 );
							$gwidth = intval( $gwidth );
							$gwidth = ( !empty( $gwidth ) ) ? $gwidth : 150;
							add_filter( 'get_avatar', 'strute_ns_filter_avatar', 10, 6 );
							echo get_avatar( $author->ID, $gwidth, '404' );
							remove_filter( 'get_avatar', 'strute_ns_filter_avatar', 10, 6 );
							?></div>
					<?php endif; ?>
...
elseif ( is_singular() ) :
	...
	if ( have_posts() ) :
		...
		while ( have_posts() ) : the_post();
			...
			$display_title = apply_filters( 'strute_loop_meta_display_title', '', 'singular' );
			if ( $display_title !== 'hide' ) :
		...
		hoot_unset_data( 'pgheadimg' );
				...
				<div <?php hoot_attr( 'loop-meta-wrap', 'singular', $wrapclasses ); ?>>
					<?php echo wp_kses_post( $image ); ?>
					<div class="hgrid">

						<div <?php hoot_attr( 'loop-meta', 'singular', 'hgrid-span-12' ); ?>>
							<div class="entry-header">
								...
								$pretitle = apply_filters( 'strute_singular_loop_pretitle', $pretitle );
								?>
								<h1 <?php hoot_attr( 'loop-title' ); ?>><?php the_title( $pretitle ); ?></h1>

								<?php
								$hide_meta_info = apply_filters( 'strute_hide_meta', false, 'top' );
								if ( !$hide_meta_info ) {
									...
									elseif (
										...
										$loc = is_page() ? hoot_get_mod( 'page_meta_location' ) : hoot_get_mod( 'post_meta_location' );
										if ( $loc == 'top' && (
											is_customize_preview() ||
											hoot_meta_info( $metarray, 'loop-meta', true )
										) ) {
											?><div <?php hoot_attr( 'loop-description' ); ?>><?php
 ...
 * Template modification Hooks
 */
do_action( 'strute_loop_meta', 'end' );