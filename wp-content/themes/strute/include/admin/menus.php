 ...
 * @return void
 */
add_action( 'init', 'strute_base_register_menus', 5 );
function strute_base_register_menus() {
	register_nav_menu( 'hoot-primary-menu', _x( 'Primary Menu', 'nav menu location', 'strute' ) );
}
 ...
 * @since 1.0
 */
function strute_nav_menu_item_custom_fields( $item_id, $item, $depth, $args ){
	$defaults = wp_parse_args( apply_filters( 'strute_nav_menu_defaults', array() ), array(
					'tagbg' => function_exists( 'hoot_get_mod' ) ? hoot_get_mod( 'accent_color' ) : '#ff8811',
					'tagfont' => function_exists( 'hoot_get_mod' ) ? hoot_get_mod( 'accent_font' ) : '#ffffff',
					'tagbg_label' => __( 'Tag Background', 'strute' ),
					'tagfont_label' => __( 'Tag Font', 'strute' ),
				) );
	?>
...
add_action( 'wp_nav_menu_item_custom_fields', 'strute_nav_menu_item_custom_fields', 10, 4 );
 ...
 * @since 1.0
 */
function strute_setup_nav_item( $menu_item ){
...
add_filter( 'wp_setup_nav_menu_item', 'strute_setup_nav_item' , 100 );
 ...
 */
function strute_update_nav_menu( $menu_id, $menu_item_db_id, $args ){
	$values = array();
	$options = array(
		'hoot_tag' => true,
		'hoot_tagfont' => true,
...
add_action( 'wp_update_nav_menu_item', 'strute_update_nav_menu', 100, 3 );
 ...
 * @since 1.0
 */
add_action( 'admin_enqueue_scripts', 'strute_enqueue_admin_styles_scripts' );
function strute_enqueue_admin_styles_scripts( $hook ) {
	if ( 'nav-menus.php' == $hook ) {
		wp_enqueue_style( 'wp-color-picker' );
		if ( file_exists( hoot_data()->incdir . 'admin/js/menuedit.js' ) )
			wp_enqueue_script( 'strute-menuedit', hoot_data()->incuri . 'admin/js/menuedit.js', array( 'wp-color-picker' ), hoot_data()->hoot_version, true );
	}
}